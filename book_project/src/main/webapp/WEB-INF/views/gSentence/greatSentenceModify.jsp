...
<script>
	$(document).ready(function(){
	      /* status 값 설정하기 */
	     var getStatus = parseInt('${great.greatContentStatus}');
	     //console.log("북플리 상태 값 : " + getStatus );
	     
	     if(getStatus == 0){
	        $("input:radio[name='greatContentStatus']:radio[value='0']").attr('checked', true);
	     }else{
	        $("input:radio[name='greatContentStatus']:radio[value='1']").attr('checked', true);
	     }
	});
</script>
...
<div class="row">
	<div class="col-4">
		<div>
			<div style="position: relative;">
				<img src="/resources/save/${great.greatImg}"
					class="bPlistCover card" id="preview" />
	...
	<div class="col-8">
		<form action="/gSentence/greatSentenceModify" method="post" enctype="multipart/form-data">
			<sec:csrfInput />
			<span id="textCount"> 0자 </span><span id="textTotal">/500자</span>

			<textarea name="greatContent" id="contents" placeholder="* 인생문장을 적어주세요👀 *" maxlength="500"> ${great.greatContent}</textarea>

			<button id="addbook" type="button" data-bs-target="#findBookModalToggle" data-bs-toggle="modal"
				class="btn btn-outline-secondary mx-auto">+ 도서 추가</button>
				
			<input type="text" id="great_no" name="great_no" style="display: none;" value="${great.great_no}" /> 
			<input type="text" id="isbn" name="greatIsbn" style="display: none;" value="${great.greatIsbn}" /> 
			<input type="text" id="bookTitle" name="greatBookTitle" value="${great.greatBookTitle}" /> <br /> <span>
				<input type="radio" value="1" name="greatContentStatus" />공개 
				<input type="radio" value="0" name="greatContentStatus" />비공개 
				<input type="file" id="file" name="greatImgFile" style="display: none;"/>
				<button type="submit" class="btn btn-outline-secondary btn-sm" style="float: right;">수정하기</button>
