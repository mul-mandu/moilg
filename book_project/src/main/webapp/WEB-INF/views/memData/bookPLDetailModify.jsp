...
<script>
   ...
   let keyword_wordlist = new Array(); 
      ...
      $.ajax({
         ...
         success: function(result) {
            ...
            for(let i = 0; i < result.length; i++){
               ...
               keyword_wordlist.push(result[i].keyword);
            }
            console.log(keyWordList);
            console.log(keyword_wordlist);
            setKeyword();  // 화면에 뿌리기
            getBookListForModif();
         },
         error: function(e){
...
///////////////////////////
   ...
   //  저장했었던 키워드 뿌리기
   function setKeyword() {
      console.log("setKeyword 진입!!!");
      // 키워드 뿌리는거 
      for(let i=0; i < keyWordList.length ; i++){
         var showlabel ="<td id='" + keyWordList[i].keyword_no +"'><span class='hashTag_td'><button id='cancelKey' type='button'><img src='/resources/img/cancel.png' width='15px' style='margin-bottom: 4px;'/></button><input type='hidden' name='keywordVal' value='"+ keyWordList[i].keyword_no +"'/><span class='selectHashTag'>"+keyword_wordlist[i]+"</span></span></td>";
         $("#finalSelecKeyword").append(showlabel);
      }
   }
      ...
      if(($("#search").val()) == "" && ($("#searchValue").val()) == ""){
         ...
         for(let i=0; i<data.length; i++){
           ...
           if(check == 1){
              ...
              // 중복체크
              for(let i=0; i<keyWordList.length; i++){
                 if(searchVal == keyWordList[i].keyword_no){
                    overlap = 1;
                 }
              }
              // 중복된게 없으면 최종 추가
              if(overlap === 1){
                 ...
                 // 최종 저장할 키워드 리스트
                 keyWordList.push({'list_no': list_no ,'keyword_no' : saveKeyWord});
 ...
 // 키워드 확인버튼 클릭시 처리 이벤트 
    ...
    $("#selectedKeyword").on("click", "#cancelKey", function(){
       console.log("키워드 삭제 버튼 클릭함!!!");
        ...
        console.log("저장한 keyWordList : " + JSON.stringify(keyWordList));
        let delVal = parseInt($(this).closest('td').attr('id'));
        let delValIdx = keyWordList.findIndex(i => i.keyword_no == delVal);
        
        console.log("삭제할 벨류 : " + delVal);
        console.log("삭제할 벨류의 인덱스 : " + delValIdx);

        for(var i = 0; i < keyWordList.length; i++){ 
          if (keyWordList[i].keyword_no === delVal) { 
           keyWordList.splice(delValIdx, 1); 
            i--; 
          }
        }
      console.log("for문 거친 keyWordList : " + JSON.stringify(keyWordList));
